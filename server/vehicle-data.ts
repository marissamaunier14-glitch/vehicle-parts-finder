...
export const YEARS: number[] = Array.from({ length: 35 }, (_, i) => new Date().getFullYear() + 1 - i);
...
export function getMakesByType(typeId: string): VehicleMake[] {
  return MAKES.filter(m => m.types.includes(typeId)).sort((a, b) => a.name.localeCompare(b.name));
}

export function getModelsByMakeAndType(makeId: string, typeId: string): VehicleModel[] {
